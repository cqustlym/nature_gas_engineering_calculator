...
目前仓库未包含自动化测试用例。建议为核心计算函数（如 `pressure.rs` 中的方法）补充单元测试，放在相应 `#[cfg(test)]` 模块下。
...
本项目当前未附带许可证文件。若需要，请添加 `LICENSE`。

---

如果你希望我：

- 添加示例 `.env` 文件模板，我可以创建一个 `env.example`；
- 添加简单的单元测试样例并在 CI 中运行，我可以帮你编写并配置；
- 或者我直接运行一次本地构建/启动并汇报结果，我可以执行（需要你允许我运行构建命令）。
