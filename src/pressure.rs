    ...
    P:  Into<f64>,
{
    ...
    info!("Calculating z with pc: {}, tc: {}, t: {}, p: {}", pc, tc, t, p);
    ...
    if ppr >= 0.1 && ppr <= 14.0 {
        ...
        for _ in 0..50 {
            ...
            let dfl = 1.0
                ...
                + (A7 / tpr.powi(3))
                * (3.0 * luopr.powi(2) + A8 * (3.0 * luopr.powi(4)))
        ...
        for _ in 0..50 {
            let fh = -0.06125 * ppr * tt * (-1.2 * (1.0 - tt).powi(2)).exp()
                ...
                * yy.powf(2.18 + 2.82 * tt);

            let dh = (1.0 + 4.0 * yy + 4.0 * yy.powi(2)
                - 4.0 * yy.powi(3) + yy.powi(4))
                / (1.0 - yy).powi(4)
...
// '杨继盛“采气工艺基础”（旧）40页
// '酸性气体修正（1986年）
pub fn niandu<Rg,Pc,Tc,T,P,Yn2,Yco2,Yh2s>(rg:Rg,pc:Pc, tc:Tc, t:T, p:P,yn2:Yn2, yco2:Yco2, yh2s:Yh2s) -> f64
where
    Rg:Into<f64>,
    Pc:Into<f64>,
    Tc:Into<f64>,
    ...
    Yh2s:Into<f64>,
{
    ...
    let k = (0.0001 * (9.4 + 0.02 * 28.97 * rg) * (9.0 * t / 5.0).powf(1.5)) / (209.0 + 19.0 * 28.97 * rg + 9.0 * t / 5.0) + kn2 + kco2 + kh2s;
    ...
    P:Into<f64>,
{
    ...
    // 使用牛顿迭代法求解luopr
    for _ in 0..30 {
        let fluopr = luopr - (0.27 * ppr / tpr) + (a1 + a2 / tpr + a3 / tpr.powi(3)) * luopr.powi(2) + 
                      (a4 + a5 / tpr) * luopr.powi(3) + (a5 * a6 * luopr.powi(6) / tpr) + 
                      (a7 * luopr.powi(3) / tpr.powi(3)) * (1.0 + a8 * luopr.powi(2)) * (-a8 * luopr.powi(2)).exp();
        let dfluopr = 1.0 + (a1 + a2 / tpr + a3 / tpr.powi(3)) * (2.0 * luopr) + 
                          (a4 + a5 / tpr) * (3.0 * luopr.powi(2)) + (a5 * a6 / tpr) * (6.0 * luopr.powi(5)) + 
    ...
    // 计算dzlt
    let dzlt = (a1 + a2 / tpr + a3 / tpr.powi(3)) + 2.0 * (a4 + a5 / tpr) * luopr + 
                5.0 * a5 * a6 * luopr.powi(4) / tpr + 
                (2.0 * a7 * luopr / tpr.powi(3)) * (1.0 + a8 * luopr.powi(2) - a8.powi(2) * luopr.powi(4)) * (-a8 * luopr.powi(2)).exp();
...
pub fn fy(rg: f64, pc: f64, tc: f64, tts: f64, tws: f64, p: f64, yn2: f64, yco2: f64, yh2s: f64, d1: f64, q: f64, ee: f64) -> f64 {
        let kn2 = yn2 * (0.00005 * rg + 0.000047) * 100.0;
        let kco2 = yco2 * (0.000078 * rg + 0.00001) * 100.0;
        let kh2s = yh2s * (0.000058 * rg - 0.000018) * 100.0;
        ...
        let k = (0.0001 * (9.4 + 0.02 * 28.97 * rg) * (9.0 * t / 5.0).powf(1.5))
                / (209.0 + 19.0 * 28.97 * rg + 9.0 * t / 5.0) + kn2 + kco2 + kh2s;
        let x = 3.5 + 986.0 / (9.0 * t / 5.0) + 0.01 * 28.97 * rg;
        let y = 2.4 - 0.2 * x;
        ...
        1.0 / ((1.14 - 2.0 * ((ee / d1 + 21.25 / re.powf(0.9)).log10() / 10.0f64.log10().log10())).powf(2.0))
    }
        ...
        EE: Into<f64>,
    {
        ...
        let tws = tws.into();
        let q = q.into();
        let mut ptf = ptf.into();
        let ee = ee.into();

        let tj = 0.8742 * q + 20.22 + 273.15;
        let mut t_avg = (tts + tws) / 2.0;

        // 第1段
        let t1 = h1 * (tws - tj) / h + tj;
        let mut pwf1 = if d1 > 0.0 {
        ...
        let mut pwf2 = if d2 > 0.0 {
            ...
            for _ in 1..=30 {
                ...
                pwf2 = (pwf1.powi(2) * (2.0_f64).powf(2.0 * s)
                    + 1.324e-10 * ffy * (q * (t1 + t2) / 2.0 * zz).powi(2)
        ...
        let pwf3 = if d3 > 0.0 {
            ...
            for _ in 1..=30 {
                ...
                pwf3 = (pwf2.powi(2) * (2.0_f64).powf(2.0 * s)
                    + 1.324e-10 * ffy * (q * (t2 + tws) / 2.0 * zz).powi(2)
                    * ((2.0_f64).powf(2.0 * s) - 1.0)
                    / d3.powi(5))
                    .sqrt();
            }
...
/// 返回:
///   井底静压，MPa
pub fn pwbs<Rg,Pc,Tc,H,Tts,Tws,Pts>(rg: Rg, pc: Pc, tc: Tc, h: H, tts: Tts, tws: Tws, pts: Pts) -> f64
where
    Rg:Into<f64>,
    Pc:Into<f64>,
    Tc:Into<f64>,
